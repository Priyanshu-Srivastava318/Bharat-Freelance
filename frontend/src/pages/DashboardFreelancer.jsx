...
export default function DashboardFreelancer() {
  ...
  const filterJobs = () => {
    let r = [...jobs];
    if (search) r = r.filter(j => j.title.toLowerCase().includes(search.toLowerCase()) || j.description.toLowerCase().includes(search.toLowerCase()) || (j.skills||[]).some(s => s.toLowerCase().includes(search.toLowerCase())));
    if (category) r = r.filter(j => j.category === category);
    if (budget) { const [mn,mx] = budget.split('-').map(Number); r = r.filter(j => j.budget >= mn && j.budget <= (mx||Infinity)); }
    setFiltered(r);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4] text-[#1a1a1a]">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 pt-28 pb-20">
        ...
        {/* Header */}
        <div className="mb-10">
          <span className="text-[11px] font-mono text-[#1dbf73] tracking-[0.3em] uppercase block mb-1">Welcome back, {user?.name?.split(' ')[0]}</span>
          <h1 className="font-display text-4xl font-bold text-[#1a1a1a]">Find Your Next Opportunity.</h1>
        </div>
        ...
        {/* Filters */}
        <div className="bg-white rounded-xl border border-black/5 shadow-sm p-5 mb-6 flex flex-col md:flex-row gap-3">
          ...
          <select value={budget} onChange={e => setBudget(e.target.value)} className="field md:w-48 !py-3 !text-sm">
                ...
                <div className="flex flex-col lg:flex-row gap-4 justify-between">
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100 font-body">🟢 Active</span>
                    </div>
                  ...
                  <div className="flex lg:flex-col gap-2 lg:min-w-[140px] items-start lg:items-stretch">
                    ...
                    <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-[#FF9933]/20 to-[#1dbf73]/20 flex items-center justify-center font-display font-bold text-[#FF9933]">
